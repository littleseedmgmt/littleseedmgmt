...
export default function StudentsPage() {
  ...
  // Group by school with stats
  const schoolStats: SchoolStudentStats[] = Object.values(
    filteredStudents.reduce((acc, student) => {
      ...
      acc[schoolId].total++
      if (student.status === 'enrolled') acc[schoolId].enrolled++
      if (student.status === 'on_leave') acc[schoolId].onLeave++
      if (student.status === 'withdrawn' || student.status === 'graduated') acc[schoolId].withdrawn++
      acc[schoolId].students.push(student)
      return acc
    }, {} as Record<string, SchoolStudentStats>)
  )
  ...
  const totals = {
    ...
    onLeave: filteredStudents.filter(s => s.status === 'on_leave').length,
    withdrawn: filteredStudents.filter(s => s.status === 'withdrawn' || s.status === 'graduated').length,
  }
    ...
    <div className="p-8">
      ...
      {showMultiSchoolView && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4 text-center">
            <p className="text-3xl font-bold text-green-600">{totals.enrolled}</p>
            <p className="text-sm text-gray-500">Enrolled</p>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-4 text-center">
            <p className="text-3xl font-bold text-amber-500">{totals.onLeave}</p>
            <p className="text-sm text-gray-500">On Leave</p>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-4 text-center">
            <p className="text-3xl font-bold text-gray-400">{totals.withdrawn}</p>
              ...
              >
                ...
                <div className="flex items-center gap-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-center">
                      <p className="text-2xl font-bold text-gray-400">{withdrawn}</p>
